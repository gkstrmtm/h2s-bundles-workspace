...
try {
    ...
    if ($ordersResponse.orders -and $ordersResponse.orders.Count -gt 0) {
        ...
        if ($jobId) {
            Write-Host "✓ job_id present in order metadata" -ForegroundColor Green
        } else {
            Write-Host "✗ job_id missing from order metadata" -ForegroundColor Red
...
if ($jobId) {
    try {
        # Use the admin portal API to get job details
        ...
        if ($jobResponse.jobs -and $jobResponse.jobs.Count -gt 0) {
            ...
            if ($job.customer_email -eq $testEmail) {
                Write-Host "  ✓ Email matches" -ForegroundColor Green
            } else {
                Write-Host "  ✗ Email mismatch: $($job.customer_email)" -ForegroundColor Red
            }
            
            if ($job.service_address -eq "456 Test Ave") {
                Write-Host "  ✓ Address matches" -ForegroundColor Green
            } else {
                Write-Host "  ✗ Address mismatch: $($job.service_address)" -ForegroundColor Red
            }
        ...
        Write-Host "✗ Error fetching job: $_" -ForegroundColor Red
    }
} else {
    Write-Host "✗ Skipping - no job_id to verify" -ForegroundColor Red
}

# ═══════════════════════════════════════════════════════════════
# STEP 4: TEST SCHEDULE APPOINTMENT (Optional)
# ═══════════════════════════════════════════════════════════════
...
if ($sessionId -and $jobId) {
    ...
    try {
        ...
        if ($scheduleResponse.ok) {
            ...
            Write-Host "✗ Schedule failed: $($scheduleResponse.error)" -ForegroundColor Red
        }
    } catch {
        Write-Host "⚠ Schedule API error (may be expected): $_" -ForegroundColor Yellow
    }
} else {
    Write-Host "✗ Skipping - missing session_id or job_id" -ForegroundColor Red
}
...
if ($jobId) {
    ...
    Write-Host "`n✓ Manual verification required in portal" -ForegroundColor Yellow
}
