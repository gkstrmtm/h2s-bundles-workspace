...
export async function POST(request: NextRequest, ctx: { params: Promise<{ offerId: string }> }) {
  try {
    ...
    const { error } = await db.from('ad_creative_links').insert({
      creative_id: creativeId,
      offer_id: offerId,
    });

    if (error) {
      // If there is a unique constraint, treat duplicates as success.
      const code = String((error as any).code || '');
      if (code === '23505') {
        return NextResponse.json({ ok: true, duplicate: true }, { headers: corsHeaders(request) });
      }

      return NextResponse.json(
        { ok: false, error: `Failed to attach creative: ${error.message}` },
