...
- Source-of-truth editor: `Dash.html` (root)
- Deploy artifact used by the `frontend/` Vercel project: `frontend/dash.html`

Routing confirmation:
- Root deployment routes `/dash` → `/Dash.html` (see `vercel.json`)
- Frontend deployment routes `/dash` → `/dash.html` (see `frontend/vercel.json`)
