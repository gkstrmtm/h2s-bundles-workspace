...
const HERO_UNDERLAY_ASSET_ID = 'de955df6-d4d9-4603-be3a-90ed2fb25e45';

let _heroUnderlayUrlPromise = null;

async function fetchHeroUnderlayUrl() {
  if (_heroUnderlayUrlPromise) return _heroUnderlayUrlPromise;

  _heroUnderlayUrlPromise = (async () => {
    try {
      ...
      // 2) Guaranteed path: fetch by asset_id from backend.
      const res = await fetch(`${PROOF_ASSET_API}?asset_id=${encodeURIComponent(HERO_UNDERLAY_ASSET_ID)}`, {
        method: 'GET',
        credentials: 'omit',
        cache: 'default'
      }).catch(() => null);
  ...
  return _heroUnderlayUrlPromise;
}
...
function findProofAssetById(slots, assetId) {
  try {
    const id = String(assetId || '').trim();
    if (!id) return null;

    const s = slots || {};
    const slotKeys = Object.keys(s);
    for (const k of slotKeys) {
      const group = s[k];
      if (!group || typeof group !== 'object') continue;
      for (const svc of ['tv_mounting', 'cameras']) {
        const arr = group[svc] || [];
        for (const a of arr) {
          if (String(a?.asset_id || '') === id) return a;
        }
      }
    }
  } catch (_) {}
...
function pickSlotAssetsByService(slotGroup, primary, secondary) {
  try {
    ...
    const p = Array.isArray(slotGroup?.[primary]) ? slotGroup[primary] : [];
    if (p.length) return p;
    const s = Array.isArray(slotGroup?.[secondary]) ? slotGroup[secondary] : [];
...
async function updateProofSections() {
  try {
    ...
    // Hero underlay (curated): apply a specific known image asset under the blue gradient.
    // This must NOT depend on proof slots including the asset.
    try {
      const heroSectionEl = document.querySelector('section.hero');
      if (heroSectionEl) {
        const url = await fetchHeroUnderlayUrl();
        if (url) {
          heroSectionEl.style.setProperty('--hero-bg-url', `url("${String(url).replace(/"/g, '%22')}")`);
        } else {
          heroSectionEl.style.removeProperty('--hero-bg-url');
        }

        // Ensure nothing else disables the hero background layers.
        heroSectionEl.classList.remove('has-proof-hero');
      }
    } catch (_) {}

    // Do not render a random hero media block; we use the curated underlay background.
    const proofHeroMedia = byId('proofHeroMedia');
    if (proofHeroMedia) proofHeroMedia.innerHTML = '';
