...
### Portal (Admin)
- Source-of-truth editor HTML (canonical): `Dash.html`
- Portal alias `/dash` is served via host-based rewrites to the canonical URL (no manual file-sync step).

Routing:
- Root deployment: `/dash` → `/Dash.html` (see `vercel.json`)
- Frontend deployment (portal host): `/dash` → `https://h2s-bundles-workspace.vercel.app/Dash.html` (see `frontend/vercel.json`)
...
Deploy guardrails:
- The deploy script runs `frontend/scripts/portal-dash-guard.mjs` automatically:
  - `pre`: validates `frontend/vercel.json` routes portal dash paths to canonical `Dash.html`
  - `post`: fetches `https://portal.home2smart.com/dash?about=1` and asserts it serves `Dash.html`
- Emergency bypass: set `H2S_SKIP_GUARDS=1`.
...
- What it does:
  ...
  - runs the canonical dash guards (pre + post)
...
- Edit `Dash.html` and deploy; do not copy it into `frontend/dash.html`.
...
- **Editing the wrong file**: edit `Dash.html` only. `frontend/dash.html` is not the canonical portal source.
